...
class PolitiClearContract extends Contract {
  ...
  async updateUsersTypes(ctx, key, name) {
    ...
    const userTypesUpdated = new UsersTypes(asset.name, asset.createdAt);
  ...
  async createVotes(ctx, key, voter, arcId, nodeId, createdAt) {
    const asset = new Votes(voter, arcId, nodeId, createdAt);
    const buffer = Buffer.from(JSON.stringify(asset));
    await ctx.stub.putState(key, buffer);
    return JSON.parse(buffer.toString());
  }

  async updateVotes(ctx, key, voter, arcId, nodeId) {
    if (voter === "" && arcId === "" && nodeId === "") {
    ...
    const votesUpdated = new Votes(asset.voter, asset.arcId, asset.nodeId, asset.createdAt);
    votesUpdated.updateVotes(voter || "", arcId || "", nodeId || "");
  ...
  async searchNodes(ctx, description) {
    ...
    for (let i = 0; i < allData.length; i++) {
      ...
      const buffer1 = await ctx.stub.getState(allData[i]);
      const asset = JSON.parse(buffer1.toString());
      finalData.push(asset);
    };
