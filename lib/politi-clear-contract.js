...
class PolitiClearContract extends Contract {
  ...
  //FORMS
  async createArcs(ctx, key, description, initialNode, initialNodeDescription, finalNode, finalNodeDescription, creatorId, createdAt, totalVotes) {
    const asset = new Arcs(description, initialNode, initialNodeDescription, finalNode, finalNodeDescription, creatorId, createdAt, totalVotes);
  ...
  async updateArcs(ctx, key, description, initialNode, initialNodeDescription, finalNode, finalNodeDescription, creatorId, totalVotes) {
    if (!(await this.dataExists(ctx, key, "Arcs"))) {
    ...
    const arcUpdated = new Arcs(
      ...
      asset.finalNodeDescription, asset.creatorId, asset.createdAt, asset.totalVotes);
      
    arcUpdated.updateArcs(description, initialNode, initialNodeDescription, finalNode, finalNodeDescription, creatorId, totalVotes);
  ...
  //FORMS
  async createForms(ctx, key, email, message, createdAt, status, response, createdBy, upgradeRequest) {
    const asset = new Forms(email, message, createdAt, status, response, createdBy, upgradeRequest);
    const buffer = Buffer.from(JSON.stringify(asset));
    await ctx.stub.putState(key, buffer);
    return JSON.parse(buffer.toString());
  }
  ...
  async updateForms(ctx, key, email, message, status, response) {
    ...
    const buffer1 = await ctx.stub.getState(key);
    const asset = JSON.parse(buffer1.toString());
    const formUpdated = new Forms(asset.email, asset.message, asset.createdAt, asset.status, asset.response, asset.createdBy);
    formUpdated.updateForms(email || "", message || "", status || "", response || "");
  ...
  async createNodes(ctx, key, description, nodeType, creatorId, createdAt) {
    const asset = new Nodes(description, nodeType, creatorId, createdAt);
    const buffer = Buffer.from(JSON.stringify(asset));
    await ctx.stub.putState(key, buffer);
    return JSON.parse(buffer.toString());
  }

  async updateNodes(ctx, key, description, nodeType) {
    ...
    const nodesUpdated = new Nodes(asset.description, asset.nodeType, asset.creatorId, asset.createdAt);
    nodesUpdated.updateNodes(description || "", nodeType || "");
  ...
  async createVotes(ctx, key, voter, arcId, vote, createdAt) {
    const asset = new Votes(voter, arcId, vote, createdAt);
    const buffer = Buffer.from(JSON.stringify(asset));
    await ctx.stub.putState(key, buffer);
    return JSON.parse(buffer.toString());
  }
