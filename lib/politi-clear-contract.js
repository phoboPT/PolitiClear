...
class PolitiClearContract extends Contract {
    ...
    async updateArcs(ctx, key, description, initialNode, finalNode, creatorId) {
        ...
        const arcUpdated = new Arcs(asset.description, asset.initialNode, asset.finalNode, asset.creatorId);
    ...
    async createNodes(ctx, key, description, nodeType, creatorId, userCreated, createdAt) {
        const asset = new Nodes(description, nodeType, creatorId, userCreated, createdAt);
        const buffer = Buffer.from(JSON.stringify(asset));
        await ctx.stub.putState(key, buffer);
        return JSON.parse(buffer.toString());
    }
    ...
    async updateNodes(ctx, key, description, nodeType, creatorId, userCreated) {
        if (description === '' && nodeType === '' && creatorId === '' && userCreated === '') {
        ...
        const nodesUpdated = new Nodes(asset.description, asset.nodeType, asset.creatorId, asset.userCreated);
        nodesUpdated.updateNodes(description || '', nodeType || '', creatorId || '', userCreated || '');
    ...
    async searchNodes(ctx, description) {
        ...
        //aqui metes o codigo de ler por ids, basta copiar um Read qualquer e tirar o filtro de type
